...
if needs_build_package ; then
  ...
  CONFIGURE_OUTPUT=$(PATH="${BISON_ROOT}"/bin:"${PATH}"\
  ...
  if ! grep -q "Building C++ Library \.* : yes" <<< "${CONFIGURE_OUTPUT}" ; then
    echo "Thrift cpp lib configuration failed."
    exit 1
  fi
  if ! grep -q "Building Python Library \.* : yes" <<< "${CONFIGURE_OUTPUT}" ; then
